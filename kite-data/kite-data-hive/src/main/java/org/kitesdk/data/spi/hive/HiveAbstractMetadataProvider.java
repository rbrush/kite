...
abstract class HiveAbstractMetadataProvider extends AbstractMetadataProvider implements
  ...
  public DatasetDescriptor update(String namespace, String name, DatasetDescriptor descriptor) {
    ...
    if (resolved != null) {
      ...
      SchemaManager manager = SchemaManager.create(conf, new Path(new Path(table.getSd().getLocation()), SCHEMA_DIRECTORY));

      DatasetDescriptor newDescriptor;

      try {
        newDescriptor = new DatasetDescriptor.Builder(descriptor).schemaUri(manager.writeSchema(descriptor.getSchema())).build();
      } catch (IOException e) {
